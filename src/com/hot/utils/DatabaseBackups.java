...
public class DatabaseBackups {
	...
	public void doBackup() {
		...
		System.out.println("------" + sdf.format(new Date()) + ", 开始备份数据库！------------\n");
		//backup();
	...
	public static void backup() {
		try {
			...
			// 调用 调用mysql的安装目录的命令
			Process child = rt
					.exec("C://Program Files (x86)//MySQL//MySQL Server 5.1//bin//mysqldump -h localhost -uroot -proot lehu");
			...
			// 要用来做导入用的sql目标文件：
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd-hh-mm-ss");
			FileOutputStream fout = new FileOutputStream("D:/mysqlBackup/" + sdf.format(new Date()) + ".sql");
	...
	public static void load1() {
		try {
			...
			// 调用 mysql 的 cmd:
			Process child = rt
					.exec("C://Program Files//MySQL//MySQL Server 5.1//bin//mysql.exe -uroot -proot dlgs_test ");
			OutputStream out = child.getOutputStream();// 控制台的输入信息作为输出流
			String inStr;
			StringBuffer sb = new StringBuffer("");
			String outStr;
